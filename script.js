...
function addremove() {
  //add remove bt-to time pair add event listener
  let processTimes = [];
  let table = document.querySelector(".main-table");
  for (let i = 0; i < process; i++) {
    let row = table.rows[2 * i + 2].cells;
...
function showGanttChart(output, outputDiv) {
  ...
  ganttChartData.sort(
    (a, b) =>
      parseInt(a[1].substring(1, a[1].length)) -
      parseInt(b[1].substring(1, b[1].length))
  );
...
function showAlgorithmChart(outputDiv) {
  ...
  let algorithmArray = ["fcfs", "sjf", "srtf", "rr", "ljf"];
...
function algo(input, utility, output) {
  ...
  while (utility.done.some((element) => element == false)) {
    let candidatesRemain = currentTimeLog.remain.filter(
      (element) => input.arrivalTime[element] <= utility.currentTime
    );
    ...
    if (currentTimeLog.running.length == 1) {
      ...
      candidates.sort((a, b) => {
        switch (input.algorithm) {
          ...
          case "srtf":
            return (
              utility.remainingBurstTime[a] - utility.remainingBurstTime[b]
            );
          case "ljf":
            return (
              utility.remainingBurstTime[b] - utility.remainingBurstTime[a]
            );
        }
      });
      found = candidates[0];
      ...
      if (utility.start[found] == false) {
        ...
        output.responseTime[found] =
          utility.currentTime - input.arrivalTime[found];
      }
    }
    utility.currentTime++;
    currentTimeLog.time++;
...
function roundrobin(input, utility, output) {
  ...
  do {
    ...
    currentTimeLog.time++;
    output.schedule.push([-1, 1]);
    firstReady = input.processId.filter(
      (element) => input.arrivalTime[element] <= utility.currentTime
    );
